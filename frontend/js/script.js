...
async function getAIAdvice() {
    ...
    try {
        const response = await fetch('http://127.0.0.1:5000/api/get-ai-advice', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function downloadReport(format) {
    ...
    try {
        const response = await fetch(`http://127.0.0.1:5000/api/download-report?type=${reportType}&format=${format}&userId=${getCurrentUserId()}`);
...
async function deleteHealthData(dataId) {
    ...
    try {
        ...
        const response = await fetch(`http://127.0.0.1:5000/api/health-data/${dataId}?user_id=${userId}`, {
            method: 'DELETE'
        });
        const result = await response.json();
...
function editHealthData(data) {
    console.log('[DEBUG] 开始编辑健康数据:', data);
    const uploadModal = document.getElementById('uploadModal');
    ...
    const uploadDataForm = document.getElementById('uploadDataForm');
    const originalHandler = uploadDataForm.onsubmit;
    
    const clonedForm = uploadDataForm.cloneNode(true);
    uploadDataForm.parentNode.replaceChild(clonedForm, uploadDataForm);
    
    const newForm = document.getElementById('uploadDataForm');
    document.getElementById('name').value = data.name;
    document.getElementById('dataType').value = data.data_type;
    document.getElementById('record_date').value = data.record_date.slice(0, 16);
    document.getElementById('manualDataValue').value = data.data_value;
    ...
    console.log('[DEBUG] 设置编辑表单提交处理程序');
    
    newForm.addEventListener('submit', async function(event) {
        event.preventDefault();
        console.log('[DEBUG] 提交编辑表单');
        ...
        try {
            ...
            console.log('[DEBUG] 发送更新请求:', payload);
            ...
            console.error('[ERROR] 更新数据时发生错误:', error);
